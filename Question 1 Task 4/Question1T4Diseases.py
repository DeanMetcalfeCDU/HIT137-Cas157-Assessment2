...
# Function to extract diseases using SciSpacy
def extract_diseases_scispacy(input_file):
    extracted_diseases = []
    ...
    try:
        ...
        for i, chunk in enumerate(chunks):
            try:
                ...
                # Extract entities labeled as 'DISEASE'
                for entity in doc.ents:
                    if entity.label_ == 'DISEASE':
                        extracted_diseases.append(entity.text)
...
# Function to extract diseases using BioBERT
def extract_diseases_biobert(input_file, model_name='alvaroalon2/biobert_diseases_ner'):
    ...
    try:
        ...
        # Split text by lines to create chunks with the aim of improving processing speed
        chunks = split_by_lines(text, lines_per_chunk=100)
...
def save_entities_to_json(entities, output_file):
    try:
        with open(output_file, 'w') as file:
            json.dump({'diseases': entities}, file, indent=4)
...
def main():
    ...
    # Extract diseases using SciSpacy
    scispacy_diseases = extract_diseases_scispacy(input_file)
    save_entities_to_json(scispacy_diseases, 'C:/Users/deanm/OneDrive/CDU/Sem2-24/HIT137/Assessments/Assessment 2/scispacy_disease_entities_full.json')

    # Extract diseases using BioBERT
    biobert_diseases = extract_diseases_biobert(input_file)
    save_entities_to_json(biobert_diseases, 'C:/Users/deanm/OneDrive/CDU/Sem2-24/HIT137/Assessments/Assessment 2/biobert_disease_entities_full.json')
